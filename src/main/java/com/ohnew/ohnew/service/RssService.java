...
public class RssService {
    ...
    public NewsByRssMultiRes fetchAndDisplayRssData() {
        try {
            ...
            List<NewsByRssRes> articles = new ArrayList<>();
            ...
            for (int i = 0; i < entries.size(); i++) {
                ...
                // DTO 생성
                NewsByRssRes article = NewsByRssRes.builder()
            ...
            NewsByRssMultiRes requestDto = NewsByRssMultiRes.builder()
                    .items(articles)
                    .build();

            // 2) Python 서버로 POST 요청
            NewsByPythonRes newsByPythonRes = webClient.post()
                    .uri("http://localhost:8000/analyze") // 실제 Python 서버 URL로 교체
                    .bodyValue(requestDto)
                    .retrieve()
                    .bodyToMono(NewsByPythonRes.class)
                    .block();

            return newsByPythonRes;
